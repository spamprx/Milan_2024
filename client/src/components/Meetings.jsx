...
export default function Meeting({
  ...
  useEffect(() => {
    ...
    const isPreferred =
      ...
      (Array.isArray(preferredTeams) &&
        preferredTeams.some(
          (team) => meeting.teams && meeting.teams.includes(team)
        )) ||
  ...
  return (
    <li
      className={`group cursor-pointer ${isPreferred ? "bg-blue-50" : ""}`}
      onClick={() => onSelect(meeting)}
    >
      <div className="mx-4 bg-[#270B5D]/[0.75] px-4 py-4 rounded-xl focus-within:bg-gray-100 hover:bg-gray-100 transition-all duration-200 ease-in-out group-hover:shadow-lg flex justify-between items-center h-24">
        <div className="min-w-0 text-left flex-grow">
          <p
            className={`text-sm font-medium ${
              isPreferred ? "text-white font-bold " : "text-black"
            }`}
          >
            {meeting.category || "Category not specified"}
          </p>
