  ...
  return (
    <div className="relative">
      ...
      {(isActive || isTransitioning) && (
        <div
          ref={dropdownRef}
          className={`absolute -top-6 -right-4 w-screen max-w-[500px] h-screen overflow-y-auto bg-[#6B5794] z-50 transition-all duration-300 rounded-l-3xl ease-in-out ${
            isActive
        ...
        >
          <div className="flex flex-col h-full justify-around p-4">
            ...
            {menuItems.map((item, index) => (
              <div
                key={index}
                className={`dropdown cursor-pointer relative flex items-center justify-between transition-all duration-300 ease-in-out ${
                  isActive
                    ? "opacity-100 translate-x-0"
                    : "opacity-0 translate-x-full"
                }`}
