    ...
    axios
      ...
      .then((response) => {
        ...
        setUser({
          ...
          Block: userData.Block ? { value: userData.Block, label: userData.Block } : null,
          interested_in: userData.interested_in,
        });
        setSelectedEvents(userData.interested_in.map(event => ({
          value: event,
          label: event
        })));
        setSubmitted(true);
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      console.log({
        name: user.name,
        email: user.email,
        selectedBlock: user.Block,
        selectedEvents: selectedEvents.map(event => event.value),
      });
