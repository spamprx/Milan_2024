...
import { startOfToday, format, isSameDay, isToday } from "date-fns";

// Lazy load components
const SportCard = lazy(() => import("../components/SportCard"));
const HomeSponsorCard = lazy(() => import("../components/HomeSponsorCard"));
const HomeLeaderboard = lazy(() => import("../components/HomeLeaderboard"));
const BlockRace = lazy(() => import("../components/BlockRace"));
const GsapAnimation = lazy(() => import("../components/Gsap"));
...
function Home() {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      try {
        const [leaderboardResponse, gamesResponse] = await Promise.all([
          fetch(
        ...
        const leaderboard = leaderboardResult.blocks.map((hostel, index) => ({
          hostel: hostel,
          points: leaderboardResult.total[index],
        }));
        ...
        if (gamesData && Object.keys(gamesData).length > 0) {
          const loadedGames = Object.entries(gamesData).flatMap(
            ([date, events]) =>
              events.map((event) => ({
                ...event,
          ...
          console.error(
            "No games data received or unexpected format:",
            gamesData
          );
          setGames([]);
        }
  ...
  const settings = {
    ...
    centerPadding: "60px",
    slidesToShow:
      currentEvents.length < 3 ? (currentEvents.length < 2 ? 1 : 2) : 3,
    speed: 500,
    dots: false,
    arrows: false,
    slidesToScroll: 1,
    ...
    responsive: [
      ...
      },
      {
        breakpoint: 1024,
        settings: {
          slidesToShow: currentEvents.length < 3 ? 2 : 3,
          centerPadding: "40px",
        },
      },
    ],
                  ...
                  <div className="absolute z-10 flex flex-col gap-6 gap1 sm:gap-10 items-center justify-center scale-90 sm:scale-100">
                    ...
                    <div className="w-[90%] sm:w-3/4 z-20 text-center text-[#D1CCB6] text-xs herotext1 sm:text-lg md:text-xl lg:text-2xl font-be-vietnam-pro">
                      <p className="font-normal mx-auto">
                        MILAN stands as one of the largest and most eagerly
                        awaited inter-hostel competitions in India. It is a
                        thrilling showcase of talent and spirit, uniting students
                        in a vibrant and dynamic celebration.
                      </p>
                      <p className="font-normal mx-auto mt-2 md:mt-6">
                        ...
                        atmosphere of heightened enthusiasm and camaraderie.
                  ...
                  <Slider {...settings}>
                    {currentEvents.map((event, index) => (
                      <div key={index} className="p-2 mb-2">
                        <Suspense fallback={<GooEffectLoader />}>
              ...
              <div className="relative w-full lg:w-1/2 aspect-square max-w-[500px] flex items-center justify-center">
                ...
                <div className="absolute w-full h-full bg-[#8F33BA] rounded-xl flex flex-col items-center justify-center text-[#D1CCB6] text-center p-4 lg:p-6">
                  ...
                  <p className="font-be-vietnam-pro font-bold text-[#D1CCB6] text-xs lg:text-lg leading-relaxed overflow-auto max-h-full mt-5">
                    Urban Rhythm captures the essence of street culture,
                    harnessing people's best abilities in a raw, engaging
                    fashion. It brings together hip-hop dance-offs🕺🏻, skateboard
                    tricks on rugged railings, and bags of spray paints🎨 for
                    the latest graffiti creations. Massive radio 📻heads and
                    hoopers🤸🏻 display their funky tricks, creating a vibrant
                    atmosphere. These streets offer people from various
                    backgrounds and communities a third space to relax with
                    friends and just be themselves. Urban Rhythm embodies a
                    mixed culture of hip-hop and punk, where rebellious kids
                    hang around corners, living their childhood to the fullest.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="relative w-full lg:w-1/2 aspect-square max-w-[500px] flex items-center justify-center order-2 sm:order-1">
                ...
                <div className="absolute w-full h-full bg-[#8F33BA] rounded-xl flex items-center justify-center text-[#D1CCB6] text-center p-4 lg:p-6">
                  <p className="font-be-vietnam-pro font-bold text-[#D1CCB6] text-sm lg:text-lg leading-relaxed overflow-auto max-h-full">
                    ...
                    Pablo, the lively mascot of Milan, ready to explore the
                    rhythm of our urban landscape! With a new look and endless
                    energy 🤸, Pablo brings a dynamic flair to every corner of
                    our college, connecting with the beat of urban life 🛹.
                    "Join us as we explore the vibrant pulse of the city
                    alongside our spirited companion!🕺🌟
                  </p>
                </div>
              </div>
            </div>
